...
namespace {
	// Combat chatter shown alongside the speaker's portrait when the
	// danger row advances. Speaker tag must match one of the portrait
	// branches below (Perhonen / Mieli / Jean / Chen / Pellegrini).
	struct ChatterLine { const char *speaker; const char *text; };
	const ChatterLine kFireChatter[] = {
		{"Perhonen",   "Fire control patterns calculated."},
		{"Perhonen",   "Their targeting solutions just locked. Hold on."},
		{"Perhonen",   "I love it when they pretend they can hit us."},
		{"Mieli",      "Plotting evasion. Stay sharp."},
		{"Mieli",      "They want to herd us. Do not let them."},
		{"Mieli",      "Match my flight or burn."},
		{"Jean",       "If I die out here I'll remember this."},
		{"Jean",       "Improvising the trajectory. Try not to vomit."},
		{"Jean",       "A gentleman thief never panics. Even now."},
		{"Chen",       "Shadows that most cannot see fire on you now."},
		{"Chen",       "The conclave watches. Run, little ship."},
		{"Pellegrini", "Their fire vector is ours. We do not flinch."},
		{"Pellegrini", "I have plotted three deaths for them. Pick one."},
	};
	const int kFireChatterCount = sizeof(kFireChatter) / sizeof(kFireChatter[0]);
}
...
CTacticalGame::CTacticalGame()
	: m_pSpline(NULL), m_pShip(NULL), m_pCruiserMesh(NULL), m_pWarmindMesh(NULL),
	  ...
	  m_animTimer(0), m_animStepDuration(0.5f), m_animStep(0),
	  m_dangerRow(-1),
	  m_gameOver(false), m_escaped(false),
	  m_sceneryGeneratedUpTo(-1),
	  m_shipCharge(0.5f), m_sailUnfurl(0.0f),
	  m_dangerPopupTimer(0.0f), m_screenShake(0.0f), m_shakeAngle(0.0f),
	  m_pPortraitPerhonen(NULL), m_pPortraitMieli(NULL), m_pPortraitJean(NULL),
	  m_pPortraitChen(NULL), m_pPortraitPellegrini(NULL),
	  m_pAsteroidTex(NULL), m_dialogueVAO(0),
	  m_pMissileMesh(NULL), m_dangerPopupLine(0),
	  m_cameraMode(CameraMode::Auto)
{}
...
void CTacticalGame::Init(CCatmullRom *pSpline, CShip *pShip,
                          ...
                          CTexture *pPortraitPerhonen, CTexture *pPortraitMieli,
                          CTexture *pPortraitJean, CTexture *pPortraitChen,
                          CTexture *pPortraitPellegrini, GLuint dialogueVAO)
{
	m_pSpline = pSpline;
	m_pShip = pShip;
	...
	m_pPortraitMieli = pPortraitMieli;
	m_pPortraitJean = pPortraitJean;
	m_pPortraitChen = pPortraitChen;
	m_pPortraitPellegrini = pPortraitPellegrini;
	...
	// Asteroid surface texture (shared between obstacle and scenery passes).
	m_pAsteroidTex = new CTexture;
	m_pAsteroidTex->Load("resources/textures/asteroid.jpg");
	m_pAsteroidTex->SetSamplerObjectParameter(GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	m_pAsteroidTex->SetSamplerObjectParameter(GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	m_pAsteroidTex->SetSamplerObjectParameter(GL_TEXTURE_WRAP_S, GL_REPEAT);
	m_pAsteroidTex->SetSamplerObjectParameter(GL_TEXTURE_WRAP_T, GL_REPEAT);
}
...
void CTacticalGame::ExecutePlan()
{
	...
	// Smooth interior waypoints with a (1, 2, 1)/4 kernel so the ship traces a
	// curve toward the final destination instead of nailing every cell centre.
	// Endpoints stay fixed: the path still starts at the origin and ends at the
	// planned destination. Two passes ≈ B-spline approximation of the polyline.
	if (m_confirmedPathWorldPos.size() >= 3) {
		for (int pass = 0; pass < 2; pass++) {
			std::vector<glm::vec3> src = m_confirmedPathWorldPos;
			for (size_t i = 1; i + 1 < src.size(); i++) {
				m_confirmedPathWorldPos[i] =
					(src[i - 1] + 2.0f * src[i] + src[i + 1]) * 0.25f;
			}
		}
	}
...
void CTacticalGame::AdvanceDangerRow()
{
	...
	m_dangerPopupTimer = 3.0f;
	m_dangerPopupLine = std::rand() % kFireChatterCount;
...
// Update
// -----------------------------------------------------------------------
void CTacticalGame::Update(float dt, GLFWwindow *window)
{
	if (m_gameOver) return;
...
void CTacticalGame::UpdateCamera(CCamera *pCamera)
{
	...
	// Resolve which camera mode to use this frame. Auto follows the
	// turn phase; F3-F5 pin a specific mode regardless of phase.
	CameraMode active = m_cameraMode;
	if (active == CameraMode::Auto) {
		if (m_phase == TurnPhase::Planning) active = CameraMode::TopDown;
		else if (m_phase == TurnPhase::Animating && !m_plannedMoves.empty())
			active = CameraMode::Animated;
		else active = CameraMode::ThirdPerson;
	}

	if (active == CameraMode::TopDown) {
		// God's eye top-down view. Ship is rendered at
	...
	} else if (active == CameraMode::Animated && !m_plannedMoves.empty()) {
	...
	// Apply screen shake — rotation-based so the skybox shakes with the foreground.
	if (m_screenShake > 0.01f) {
		glm::vec3 camPos = pCamera->GetPosition();
		glm::vec3 lookAt = pCamera->GetView();
		glm::vec3 forward = lookAt - camPos;
		float dist = glm::length(forward);
		forward = glm::normalize(forward);

		float maxAngle = glm::radians(m_screenShake * 6.0f); // up to 6 degrees
		glm::vec2 shakeTarget(
			maxAngle * ((float)(rand() % 200 - 100) / 100.0f),
			maxAngle * ((float)(rand() % 200 - 100) / 100.0f));
		m_shakeAngle = glm::mix(m_shakeAngle, shakeTarget, 0.3f);

		glm::vec3 right = glm::normalize(glm::cross(forward, glm::vec3(0, 1, 0)));
		glm::vec3 up = glm::cross(right, forward);
		glm::mat4 rot = glm::rotate(glm::mat4(1.0f), m_shakeAngle.x, right);
		rot = glm::rotate(rot, m_shakeAngle.y, up);
		glm::vec3 newLookAt = camPos + glm::vec3(rot * glm::vec4(forward, 0.0f)) * dist;
		pCamera->Set(camPos, newLookAt, pCamera->GetUpVector());
	} else {
                            ...
                            CFreeTypeFont *pFont, int screenWidth, int screenHeight)
{
	...
	// Sun light anchored to camera so the incident direction matches the visible sun
	// regardless of where the ship is along the spline.
	glm::vec3 sunDirWorld = glm::normalize(glm::vec3(-100, 100, -100));
	glm::vec4 lightPos(pCamera->GetPosition() + sunDirWorld * 10000.0f, 1.0f);
	pMainProgram->SetUniform("light1.position", viewMatrix * lightPos);
	pMainProgram->SetUniform("light1.La", glm::vec3(0.35f, 0.32f, 0.25f));
	pMainProgram->SetUniform("light1.Ld", glm::vec3(1.6f, 1.30f, 0.85f));
	pMainProgram->SetUniform("light1.Ls", glm::vec3(1.4f, 1.20f, 0.80f));
	...
	pMainProgram->SetUniform("bUseTexture", true);
	pMainProgram->SetUniform("sampler0", 0);
	if (m_pAsteroidTex) m_pAsteroidTex->Bind(0);
	pMainProgram->SetUniform("material1.Ma", glm::vec3(0.25f, 0.2f, 0.15f));
	pMainProgram->SetUniform("material1.Md", glm::vec3(0.7f, 0.6f, 0.5f));
                                  ...
                                  glm::mat4 viewMatrix)
{
	...
	glm::mat4 shipMV = ms.Top() * orient * glm::scale(glm::mat4(1.0f), glm::vec3(0.6f));
	...
	pHullProgram->SetUniform("light1.La", glm::vec3(0.35f, 0.32f, 0.25f));
	pHullProgram->SetUniform("light1.Ld", glm::vec3(1.6f, 1.30f, 0.85f));
	pHullProgram->SetUniform("light1.Ls", glm::vec3(1.4f, 1.20f, 0.80f));
	...
	if (m_sailUnfurl > 0.01f) {
		...
		pSailProgram->SetUniform("light1.La", glm::vec3(0.35f, 0.32f, 0.25f));
		pSailProgram->SetUniform("light1.Ld", glm::vec3(1.6f, 1.30f, 0.85f));
		pSailProgram->SetUniform("light1.Ls", glm::vec3(1.4f, 1.20f, 0.80f));
                                     ...
                                     glm::mat4 viewMatrix)
{
	...
	pMainProgram->SetUniform("material1.Md", glm::vec3(0.6f, 0.55f, 0.5f));
                               ...
                               CFreeTypeFont *pFont, int screenWidth, int screenHeight)
{
	...
	// Combat chatter popup when danger row advances — random speaker per advance
	if (m_dangerPopupTimer > 0.0f) {
		const ChatterLine &line = kFireChatter[m_dangerPopupLine % kFireChatterCount];
		CTexture *speakerTex = m_pPortraitPerhonen;
		glm::vec3 speakerColour(0.3f, 1.0f, 0.6f); // perhonen default
		if (strcmp(line.speaker, "Mieli") == 0) {
			speakerTex = m_pPortraitMieli;
			speakerColour = glm::vec3(0.4f, 0.8f, 1.0f);
		} else if (strcmp(line.speaker, "Jean") == 0) {
			speakerTex = m_pPortraitJean;
			speakerColour = glm::vec3(1.0f, 0.84f, 0.0f);
		} else if (strcmp(line.speaker, "Chen") == 0) {
			speakerTex = m_pPortraitChen;
			speakerColour = glm::vec3(0.9f, 0.15f, 0.15f);
		} else if (strcmp(line.speaker, "Pellegrini") == 0) {
			speakerTex = m_pPortraitPellegrini;
			speakerColour = glm::vec3(0.85f, 0.3f, 0.85f);
		}
		// Fallback to Perhonen if a portrait failed to load
		if (!speakerTex) speakerTex = m_pPortraitPerhonen;
		...
		if (speakerTex) {
			...
			// Right-align text against the portrait's left edge so lines
			// never bleed onto the face, regardless of length.
			int gapToPortrait = 16;
			int textRight = px - gapToPortrait;
			int nameW = pFont->GetTextWidth(line.speaker, 16);
			int textW = pFont->GetTextWidth(line.text, 18);
			int textX = textRight - textW;
			int nameX = textRight - nameW;
			int nameY = py + portraitSize / 2 + 30;
			int textY = py + portraitSize / 2 + 8;

			// Translucent backdrop panel sized to the wider of the two lines
			int panelW = std::max(nameW, textW) + 24;
			int panelX = textRight - panelW + 12;
			int panelY = textY - 8;
			int panelH = (nameY + 16) - panelY + 6;
			fontProgram->SetUniform("vColour", glm::vec4(0.05f, 0.05f, 0.08f, 0.7f * popupAlpha));
			glm::mat4 panelMV = glm::translate(glm::mat4(1.0f), glm::vec3((float)panelX, (float)panelY, 0.0f))
				* glm::scale(glm::mat4(1.0f), glm::vec3((float)panelW, (float)panelH, 1.0f));
			fontProgram->SetUniform("matrices.modelViewMatrix", panelMV);
			glBindTexture(GL_TEXTURE_2D, 0); // untextured
			glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

			// Speaker name (top, in their colour)
			fontProgram->SetUniform("vColour", glm::vec4(speakerColour, popupAlpha));
			pFont->Render(nameX, nameY, 16, line.speaker);
			// Line of dialogue (bottom, near-white)
			fontProgram->SetUniform("vColour", glm::vec4(0.95f, 0.95f, 0.95f, popupAlpha));
			pFont->Render(textX, textY, 18, line.text);
		}
	}
