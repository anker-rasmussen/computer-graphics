...
void CShip::CreateSolarSail(float sailHeight, float sailWidth, float curvature)
{
	...
	// Bow offset curves the Bezier edges outward
	...
	// Coons patch: P(s,t) = ruled_s + ruled_t - bilinear + concavity dip
	auto evalPos = [&](float sv, float t) -> glm::vec3 {
		...
		// billowing effect -- zero at edges, max at centre
	...
	// Reference normal at patch centre for consistent orientation
...
void CShip::Create(string directory, string filename)
{
	...
	// Attrib pointers -- stride matches the interleaved layout: pos|norm|uv|col
