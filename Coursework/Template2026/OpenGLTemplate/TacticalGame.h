...
// Camera mode override. Auto = follow the turn phase (default).
// The other values are explicit modes the user can pin via F3-F5.
...
private:
	...
	CTexture *m_pPortraitMieli;
	CTexture *m_pPortraitJean;
	CTexture *m_pPortraitChen;
	CTexture *m_pPortraitPellegrini;
	CTexture *m_pAsteroidTex;  // owned: loaded in Init()
	...
	int m_dangerPopupLine;    // index into the static chatter table

	// Manual camera override (F3-F5 in Game::KeyCallback). Auto follows phase.
	CameraMode m_cameraMode;
public:
	void SetCameraMode(CameraMode m) { m_cameraMode = m; }
	CameraMode GetCameraMode() const { return m_cameraMode; }
private:

	// Screen shake — rotation-based so the skybox shakes coherently
	float m_screenShake;          // 0-1 intensity, decays over time
	glm::vec2 m_shakeAngle;       // current pitch/yaw offset (radians)
