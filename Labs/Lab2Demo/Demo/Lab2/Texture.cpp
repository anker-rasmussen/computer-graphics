...
void CTexture::CreateFromData(unsigned char* bData, int iWidth, int iHeight, int iBPP, GLenum format, bool bGenerateMipMaps)
{
...
bool CTexture::Load(string sPath, bool bGenerateMipMaps)
{
	...
	if(!dib) {
		std::cerr << "Cannot load image: " << sPath << std::endl;
		return false;
	}
