...
bool CShader::LoadShader(string sFile, int iType)
{
	...
	if(!GetLinesFromFile(sFile, false, &sLines)) {
		std::cerr << "Cannot load shader: " << sFile << std::endl;
		return false;
	}
	...
	if(iCompilationStatus == GL_FALSE)
	{
		...
		std::cerr << "Error in " << sShaderType << "!" << std::endl
		          << sFile << std::endl
		          << "Shader file not compiled. The compiler returned:" << std::endl
		          << sInfoLog << std::endl;
		return false;
	}
	m_iType = iType;
...
bool CShaderProgram::LinkProgram()
{
	...
	if (iLinkStatus == GL_FALSE)
	{
		...
		std::cerr << "Error! Shader program wasn't linked! The linker returned:" << std::endl
		          << sInfoLog << std::endl;
		return false;
	}
